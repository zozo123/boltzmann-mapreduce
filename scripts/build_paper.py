...
PDF_NAME = "evidence-aware-reduction.pdf"
SOURCE_DATE_EPOCH = "1784246400"  # 2026-07-17 UTC; stable PDF metadata.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmr-paper-") as tmp:
        build = Path(tmp) / "submission"
        build.mkdir()
        for name in ("main.tex", "body.tex", "fig_contract.tex", "refs.bib"):
            shutil.copy2(SUBMISSION / name, build / name)
        ...
        warning_indexes = [i for i, line in enumerate(log_lines) if forbidden.search(line)]
        if warning_indexes:
            details = "\n---\n".join(
                "\n".join(log_lines[max(0, i - 2) : i + 3]) for i in warning_indexes
            )
            raise SystemExit(f"paper log contains release-blocking warnings:\n{details}")

        built_pdf = build / "main.pdf"
        ...
        destinations = (
            ROOT / "output" / "pdf" / PDF_NAME,
            ROOT / "docs" / PDF_NAME,
            ROOT / "submission" / PDF_NAME,
            # Backward-compatible aliases. Keeping them byte-identical avoids serving
            # stale public versions through historical repository links.
    ...
    canonical = destinations[0].read_bytes()
    if any(destination.read_bytes() != canonical for destination in destinations[1:]):
